...
def power_on_pwm_driver():
    io.output(relay_pin, io.HIGH)
    print("PWM driver powered on")

# Fungsi untuk mematikan relay (memutus catu daya 5V ke PWM driver)
def power_off_pwm_driver():
    io.output(relay_pin, io.LOW)
    print("PWM driver powered off")
...
def identifikasi_sampah():
    while current_page == 3:
        ...
        print("Menahan Botol selama 10 detik")
        time.sleep(10)
        ...
        if sensor1 == io.LOW:
            ...
            print("Botol kecil telah dikeluarkan")
            ...
            print("Membuka pintu agar botol ditimbang")
            for angle in range (0, 181, 1):
            ...
            print("Botol telah dijatuhkan ke loadcell")
            for angle in range (180, -1, -1):
                kit.servo[1].angle = angle
                time.sleep(0.01)
        else:
...
def menimbang():
    ...
    #======#
def menambah():
    io.input(pushbutton_merah) == io.LOW        
...
def show_page(page_func):
    ...
    if current_page == 2:  # Assuming langkah_2 pada index 2
        ...
        servo_thread.start()
    if current_page == 3:
        relay_thread = threading.Thread(target= power_on_pwm_driver)
        relay_thread.start()
        ...
        tambah_thread.start()
    if current_page == 5:
        relay_thread == threading.Thread(target= power_off_pwm_driver)
        relay_thread.start()
...
def langkah_2():
    ...
    page_frame = Frame(window)
    page_frame.grid()
    label1 = Label(page_frame, text = "Langkah 2\n Memasukkan Sampah Botol Plastik", fg = "black", bg="#88BC59", font= ("Tahoma", 16, "bold"),bd = 2,justify="center")
    ...
    label3 = Label(page_frame, text = (
        ...
        "4. Jika telah lewat 40 detik, maka pintu akan tertutup secara otomatis"), wraplength=950, fg= "black", font = ("Tahoma", 12), bd = 2, bg = "#9DC183",justify="left" )
...
def langkah_3():
    ...
    label_sampah_terkumpul = Label(page_frame, text = "Sampah Anda terkumpul : 0", fg = "black", font=("Tahoma", 12, "bold"), bg = "#9DC183",justify="center")
...
def penutup():
    ...
    label3 = Label(page_frame, text = "Rp. "+poinku+"", fg = "black" ,bg="#ffe14c",font= ("Tahoma", 16,"bold"), justify="center")
    ...
    label4 = Label(page_frame, text = (
        ...
        "SAMPAI JUMPA LAGI\n"
        "SEMOGA HARI ANDA MENYENANGKAN"), fg = "black" ,bg="#9DC183",font= ("Tahoma", 12,"italic"), justify="center")
...
def akhir_page():
    ...
    label1 = Label(page_frame, text="Anda Berhasil Logout", fg = "black", bg = "#88BC59", font=("Tahoma", 18, "bold"), bd=2, justify="center")
    label1.grid(column=0, row=0, columnspan=3, padx=300, pady=20)
